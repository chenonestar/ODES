...
有两处状态不是请求的直接结果，而是由后台推进器驱动的（`cmd/eval` 的
`statusTicker`，**30 秒一次**）：

- `published → running`：到达 `start_at` 后自动迁移
- `running → closed`：到达 `end_at` 后自动迁移

涉及这两处时必须用 `L.waitForBadge()` 显式等待。写成"刷一次看看"就是一个
纯看机器快慢的 flaky 测试——本地慢、等到了所以是绿的，CI 快、没等到就红，
而且红的位置每次还不一样。`lifecycle.js` 在 CI 上红过一次，就是这个原因。
