...
//     一旦写入日志即构成身份线索。此处仅在 -v 模式下打印到控制台，
//     供尖刺阶段观察，正式版本不得输出。
...
func (s *DHCPServer) handle(pkt []byte) []byte {
	...
	case dhcpDiscover:
		...
		s.logf("DHCP DISCOVER %s → OFFER %s%s", mac, ip, hostnameHint(opts))
...
// hostnameHint 仅用于尖刺阶段的控制台观察。
// 正式版本必须删除——主机名常含机主姓名，属身份线索（NFR-ANO-040）。
func hostnameHint(opts map[byte][]byte) string {
	if h, ok := opts[optHostname]; ok && len(h) > 0 {
		return fmt.Sprintf("  [hostname=%q ← 正式版不得记录]", string(h))
	}
	return ""
}
