...
`report.ttf` **已随仓库提交**，克隆下来直接 `go build` 就能出 PDF，
不需要任何额外步骤。

- 字体：Noto Serif CJK SC 2.002，转成 glyf 轮廓（转换脚本见 `tools/otf2ttf/`）
- 许可：SIL OFL 1.1，全文见同目录 `LICENSE-OFL.txt`
- 体积：30.1 MB，git 压缩后约 15.5 MB

若要换成别的字体，替换本文件即可；换之前先跑一遍下面的自查。
缺字体或字体不合格时：PDF 导出会给出明确提示并拒绝出报告，
HTML / xlsx / CSV 三个出口不受影响，照常可用。
